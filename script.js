...
async function submitReview() {
    console.log("¡Clic en Enviar detectado!"); // <--- ESTO ES PARA DEPURAR
    ...
    // Construimos los datos como formulario, no como JSON
    ...
    try {
        ...
        // Como usamos no-cors, asumimos éxito si llega aquí
...
function toggleTheme() {
    ...
    btn.innerHTML = document.body.classList.contains('dark-theme') ? '<i class="fa-solid fa-sun"></i>' : '<i class="fa-solid fa-moon"></i>';
}
...
function renderSkeletons() {
    ...
    grid.innerHTML = Array(4).fill(0).map(() => `
        <div class="card"><div class="skeleton" style="height: 180px;"></div><div class="skeleton"></div></div>
    `).join('');
}
...
function initMap(businesses) {
    if (map) map.remove();
    map = L.map('map').setView([20.52, -103.22], 13);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);
    businesses.forEach(b => {
        if(b.lat && b.lng) {
            L.marker([b.lat, b.lng]).addTo(map).bindPopup(b.nombre);
        }
    });
}
...
function filterBusinesses() {
    ...
    const filtered = localBusinesses.filter(b => {
        // Asegúrate de que b.nombre y b.descripcion existan
        const name = b.nombre ? b.nombre.toLowerCase() : "";
        const desc = b.descripcion ? b.descripcion.toLowerCase() : "";
        const cat = b.categoria ? b.categoria.toLowerCase() : "";
        
        const matchSearch = name.includes(searchVal) || desc.includes(searchVal);
        const matchCat = catVal === "todos" || cat === catVal.toLowerCase();
...
function openReviewsModal(busId, busName) {
    const reviewsList = document.getElementById("reviews-list");
    const modal = document.getElementById("view-reviews-modal");
    
    // Filtrar reseñas para el negocio seleccionado
    // Aseguramos que los IDs sean strings para evitar errores de comparación
    const filteredReviews = allReviews.filter(r => String(r.id_negocio).trim() === String(busId).trim());
    
    // Mostrar modal
    modal.style.display = "flex";
    reviewsList.innerHTML = `<h3>Opiniones sobre ${busName}</h3>`;

    if (filteredReviews.length === 0) {
        reviewsList.innerHTML += "<p>Aún no hay reseñas para este negocio. ¡Sé el primero en calificar!</p>";
        return;
    }
    ...
    filteredReviews.forEach(r => {
        reviewsList.innerHTML += `
            <div class="review-item">
                <div class="review-stars">${generateStars(r.calificacion)}</div>
                <p>${r.comentario || "Sin comentario"}</p>
                <small>${r.fecha ? new Date(r.fecha).toLocaleDateString() : ""}</small>
            </div>
        `;
    });
}
